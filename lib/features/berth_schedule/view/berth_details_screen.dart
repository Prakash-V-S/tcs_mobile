...
class BerthDetailsScreen extends StatelessWidget {
  ...
  const BerthDetailsScreen({Key? key, required this.schedule}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: Colors.white, // Lighter background color conforming to mockup
      appBar: AppBar(
        ...
        title: const Text('Vessel Details', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold, fontSize: 18)),
        centerTitle: false,
      ),
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
             ...
             Row(
               children: [
                 const Icon(Icons.directions_boat_outlined, color: Colors.blue, size: 28),
             ...
             // VOYAGE Section
             const Text('VOYAGE', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.grey, letterSpacing: 1.2)),
             const SizedBox(height: 16),
             Row(
               children: [
             ...
             // SCHEDULE Section
             const Text('SCHEDULE', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.grey, letterSpacing: 1.2)),
             const SizedBox(height: 16),
             Row(
               children: [
                 Expanded(
                   child: Column(
                     children: [
                       _buildScheduleBox('ETA', schedule.eta, 'ATA', schedule.ata),
                     ],
                   )
                 ),
                 const Padding(
                 ...
                 Expanded(
                   child: Column(
                     children: [
                       _buildScheduleBox('ETD', schedule.etd, 'ATD', schedule.atd),
                     ],
                   )
                 ),
               ],
  ...
  Widget _buildScheduleBox(String label1, String val1, String label2, String val2) {
    return Container(
      padding: const EdgeInsets.all(16),
      ...
      child: Column(
        ...
        children: [
          ...
          Text((val1 == 'null') ? 'N/A' : val1, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14)),
          const SizedBox(height: 16),
          Text(label2, style: const TextStyle(color: Colors.grey, fontSize: 12)),
          const SizedBox(height: 4),
          Text((val2 == 'null') ? 'N/A' : val2, style: const TextStyle(color: Colors.black87, fontSize: 14)),
        ],
      ),
    );
  }
